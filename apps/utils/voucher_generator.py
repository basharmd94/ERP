...
def generate_voucher_number(zid: int, prefix: str, table: str, column: str, length: int = 6) -> str:
    """
    ...
    try:
        with transaction.atomic():
            ...
            with connection.cursor() as cursor:
                # Lock the table and get the last voucher number
                sql = f"""
                    SELECT {column}
                    FROM {table}
                ...
                cursor.execute(sql, [zid, f"{prefix}%"])
